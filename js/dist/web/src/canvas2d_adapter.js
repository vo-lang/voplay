const FRAME_MAGIC = [0x56, 0x46, 0x43, 0x31];
export class Canvas2dGpuAdapter {
    #deviceGeneration;
    #maxCommands;
    #maxCommandBytes;
    #records = new Map();
    ...
    registerTexture(texture, source, engine) {
        this.#assertOpen();
        if (texture <= 0n)
            throw new RangeError("invalid Canvas2D texture");
        if (engine !== undefined)
    ...
    removeTexture(texture, engine) {
        ...
        if (engine !== undefined)
            validateHandle(engine);
        return this.#textures.delete(textureKey(engine === undefined ? "global" : handleScope(engine), texture));
    }
    attach(canvas, metrics) {
        this.#assertOpen();
        if (this.#records.has(canvas))
            throw new Error("Canvas2D surface already attached");
    ...
    resize(canvas, metrics) {
        ...
        if (record.pendingFence !== undefined)
            throw new Error("Canvas2D frame pending");
    ...
    submit(canvas, commands, graphSignature, engine, renderRevision) {
        ...
        rasterPortableFrame(record.backbuffer, portableFrameCommands(commands), this.#maxCommands, textures);
    ...
    present(canvas, fenceValue) {
        ...
        if (record.pendingFence !== fenceValue)
            throw new Error("Canvas2D fence mismatch");
    ...
    detach(canvas) {
        ...
        const record = this.#record(canvas);
        if (record.pendingFence !== undefined)
            throw new Error("Canvas2D frame outcome unknown");
    ...
    close() {
        if (this.#closed)
            return;
        for (const canvas of [...this.#records.keys()])
            this.abandon(canvas);
        this.#textures.clear();
        this.#closed = true;
    }
    #record(canvas) {
        ...
        if (record === undefined)
            throw new Error("unknown Canvas2D surface");
        return record;
    }
    #assertOpen() {
        if (this.#closed)
            throw new Error("Canvas2D adapter closed");
    }
}
export function portableFrameCommands(bytes) {
    ...
    if (bytes.byteLength < headerBytes)
        throw new RangeError("truncated Voplay 3D scene frame");
    const fallbackBytes = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(8, true);
...
export function rasterPortableFrame(target, bytes, maxCommands, textures = new Map()) {
    ...
    for (let index = 0; index < FRAME_MAGIC.length; index += 1) {
        if (bytes[index] !== FRAME_MAGIC[index])
            throw new RangeError("invalid portable frame magic");
    }
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const count = view.getUint32(4, true);
    if (count > maxCommands)
        throw new RangeError("portable frame command capacity");
    const context = target.getContext("2d", { alpha: true });
    if (context === null)
        throw new Error("Canvas2D backbuffer unavailable");
    let offset = 8;
    for (let index = 0; index < count; index += 1) {
        ...
        switch (tag) {
            ...
            case 3: {
                ...
                drawTintedImage(context, source, sourceX, sourceY, sourceWidth, sourceHeight, x, y, width, height, tint);
                break;
            }
            default:
    ...
    if (offset !== bytes.byteLength)
        throw new RangeError("portable frame trailing bytes");
}
function drawTintedImage(context, source, sourceX, sourceY, sourceWidth, sourceHeight, x, y, width, height, tint) {
    ...
    if (scratchContext === null)
        throw new Error("Canvas2D sprite scratch unavailable");
    scratchContext.drawImage(source, sourceX, sourceY, sourceWidth, sourceHeight, 0, 0, width, height);
...
function textureKey(scope, texture) {
    return `${scope}/${texture}`;
}
function textureIdentity(key, scope) {
    const prefix = `${scope}/`;
