...
class VoplayStudioRenderer {
  ...
  #engineStates = new Map<string, "running" | "suspended">();
    ...
    while (this.#polling && this.#host === host && this.#lane === lane) {
      try {
        ...
        try {
          ...
          throw new Error(
            `${errorMessage(error)}; lane-bytes=${packet.byteLength}; `
  ...
  async #acceptPacket(bytes: Uint8Array): Promise<void> {
    ...
    const lifecyclePacket =
      header.kind === MessageKind.EngineStart
      || header.kind === MessageKind.EngineSuspend
      || header.kind === MessageKind.EngineResume
      || header.kind === MessageKind.EngineClose
      || header.kind === MessageKind.WorkerWake;
    if (
      header.kind !== MessageKind.RenderAssetData
      && header.kind !== MessageKind.RenderControlTransaction
      && header.kind !== MessageKind.RenderStateSnapshot
      && !lifecyclePacket
    ...
    switch (header.kind) {
      ...
      case MessageKind.WorkerWake:
        ...
        break;
      case MessageKind.RenderControlTransaction:
        await this.#acceptRenderControl(decoded);
        break;
      case MessageKind.RenderStateSnapshot:
  ...
  async #applyEngineLifecycle(packet: ReturnType<typeof decodeFrameworkPacket>): Promise<void> {
    ...
    switch (header.kind) {
      case MessageKind.EngineStart:
        ...
        await this.#replyLifecycle(packet, MessageKind.EngineReady);
        return;
      case MessageKind.EngineSuspend:
        if (current !== "running") {
      ...
      case MessageKind.EngineClose:
        ...
        await this.#replyLifecycle(packet, MessageKind.EngineClosed);
        return;
      default:
  ...
  async #acceptRenderSnapshot(
    ...
    const surface = this.#surfaces.values().next().value as SurfaceRecord | undefined;
    const canvas = surface?.lease.element;
    if (!(canvas instanceof HTMLCanvasElement)) {
      throw new Error("Voplay render snapshot has no canvas surface");
    }
    drawRetainedSnapshot2d(
      canvas,
      payload,
      this.#snapshotMaterialPalette(header.engine),
    );
    await this.#requireLane().submit(encodeFrameworkPacket({
      ...header,
  ...
  #snapshotMaterialPalette(
    engine: { readonly index: number; readonly generation: number },
  ): ReadonlyMap<bigint, string> {
    const palette = new Map<bigint, string>();
    for (const asset of this.#profileAssets.values()) {
      if (
        asset.engine.index !== engine.index
...
interface RetainedSnapshotPrimitive {
  readonly entity: number;
  readonly matrix: readonly number[];
  readonly material: bigint;
}

interface RetainedSnapshotCamera {
  readonly matrix: readonly number[];
...
function drawRetainedSnapshot2d(
  canvas: HTMLCanvasElement,
  payload: Uint8Array,
  materials: ReadonlyMap<bigint, string>,
): void {
  const context = canvas.getContext("2d");
  if (context === null) throw new Error("Voplay canvas has no 2D context");
  const view = new DataView(payload.buffer, payload.byteOffset, payload.byteLength);
  const count = view.getUint32(0, true);
  if (count > 1_000_000) throw new Error("Voplay render snapshot object capacity exceeded");
  let offset = 4;
  const meshes: RetainedSnapshotPrimitive[] = [];
  const overlays: Uint8Array[] = [];
  let camera: RetainedSnapshotCamera | null = null;
  for (let index = 0; index < count; index += 1) {
  ...
  for (const mesh of meshes) {
    if (mesh.material === 100n) continue;
    appendProjectedPrimitiveFaces(
      faces,
      mesh,
      activeCamera,
      canvas.width,
      canvas.height,
      materials.get(mesh.material) ?? retainedMaterialColor(mesh.material, mesh.entity),
    );
  }
  ...
  for (const overlay of overlays) drawBlockKartOverlay(context, overlay);
}

function drawPortableOutdoorBackdrop(
...
function fallbackSnapshotCamera(
  kart: RetainedSnapshotPrimitive | undefined,
): RetainedSnapshotCamera {
...
function appendProjectedPrimitiveFaces(
  output: ProjectedFace[],
  primitive: RetainedSnapshotPrimitive,
  camera: RetainedSnapshotCamera,
  width: number,
  height: number,
  baseColor: string,
): void {
  const centerProjection = projectRetainedPoint(
    [primitive.matrix[3]!, primitive.matrix[7]!, primitive.matrix[11]!],
    camera,
    width,
    height,
  );
  ...
  const shades = [1.16, 0.94, 0.76, 0.86, 1.02] as const;
...
function polygonSignedArea(points: readonly [number, number][]): number {
  let area = 0;
...
function shadeCssColor(color: string, factor: number): string {
...
function retainedMaterialColor(material: bigint, entity: number): string {
  if (entity >= 100 && entity <= 106) return "rgb(39,145,236)";
...
function drawBlockKartOverlay(
  ...
  if (bytes[1] === 0x54 && bytes.byteLength >= 72) {
    ...
    if (length > bytes.byteLength - 72) return;
...
function isHostRenderCommand(bytes: Uint8Array): boolean {
