...
export class WebGpuRetainedRenderer {
    ...
    constructor(canvas, device, context, format) {
        ...
        this.#doubleSidedPipeline = createScenePipeline(device, format, sceneLayouts.pipeline, "Voplay retained 3D double-sided pipeline", "none", false, true);
        this.#blendPipeline = createScenePipeline(device, format, sceneLayouts.pipeline, "Voplay retained 3D blend pipeline", "none", true, false);
    ...
    #prepareScene() {
        ...
        const previousBatches = new Map(this.#preparedBatches.map((batch) => [
    ...
    async #draw(alpha, validate) {
        ...
        try {
            ...
            if (this.#shadowViewProjection === null || redrawShadow) {
                this.#shadowViewProjection = sceneLightViewProjection(cameraPosition, this.#scene.sun.direction);
            }
            ...
            for (const batch of this.#preparedBatches) {
                ...
                for (let index = 0; index < batch.instances.length; index += 1) {
                    writePreparedInstance(batch.values, index * INSTANCE_FLOATS, batch.instances[index], alpha);
                }
    ...
    #materialPipeline(material) {
        ...
        if (material?.doubleSided === true)
            return this.#doubleSidedPipeline;
            ...
            if (asset.kind === 2) {
                ...
                try {
                    ...
                    for (let mipLevel = 0; mipLevel < mipLevelCount; mipLevel += 1) {
                        ...
                        try {
                            this.#device.queue.copyExternalImageToTexture({ source }, { texture, mipLevel }, [width, height, 1]);
                        }
                        finally {
                            if (source !== bitmap)
                                source.close();
...
function createScenePipeline(device, format, pipelineLayout, label, cullMode, blend, depthWriteEnabled) {
...
function decodeScene(objects, cache) {
    ...
    directionalLights.sort((left, right) => Number(right.castsShadow) - Number(left.castsShadow)
    ...
    const fill = directionalLights[1] ?? {
...
function interpolateTransformMatrix(previous, current, alpha) {
    ...
    const rotation = slerpQuaternion(previousTransform.rotation, currentTransform.rotation, alpha);
...
function interpolateScalar(previous, current, alpha) {
...
function decomposeTransformMatrix(matrix) {
    ...
    if (scaleX < 1e-8 || scaleY < 1e-8 || scaleZ < 1e-8)
        return null;
...
function slerpQuaternion(previous, current, alpha) {
    let target = current;
...
function composeTransformMatrix(rotation, scale, translation) {
...
function writePreparedInstance(output, offset, instance, alpha) {
    const matrix = interpolateTransformMatrix(instance.previousMatrix, instance.currentMatrix, alpha);
...
function sceneLightViewProjection(cameraPosition, lightDirection) {
