export interface RetainedGpuAsset {
...
export interface RetainedGpuObject {
...
interface GpuRenderPassLike {
  ...
  setIndexBuffer(buffer: unknown, format: "uint32"): void;
...
interface ResidentMesh {
  readonly revision: bigint;
  readonly vertex: GpuBufferLike;
  readonly index: GpuBufferLike;
  readonly indexCount: number;
  instance: GpuBufferLike;
...
interface ResidentMaterial {
  ...
  readonly textures: readonly bigint[];
  readonly alphaMode: number;
  readonly doubleSided: boolean;
  readonly parameters: GpuBufferLike;
  bindGroup: unknown | null;
  boundTextureRevisions: readonly bigint[];
}
...
interface ResidentInstanceBatch {
...
export class WebGpuRetainedRenderer {
  ...
  readonly #shadowUniform: GpuBufferLike;
  readonly #shadowUniformBindGroup: unknown;
  readonly #shadowTexture: GpuTextureLike;
  readonly #shadowSampler: unknown;
  readonly #sampler: unknown;
  readonly #whiteTexture: GpuTextureLike;
  readonly #whiteTextureView: unknown;
  readonly #fallbackMaterialParameters: GpuBufferLike;
  ...
  #shadowViewProjection: Float32Array | null = null;
  #closed = false;
  ...
  static async create(canvas: HTMLCanvasElement): Promise<WebGpuRetainedRenderer> {
    ...
    if (gpu === undefined) throw new Error("Voplay retained 3D requires WebGPU");
    const adapter = await gpu.requestAdapter({ powerPreference: "high-performance" });
    if (adapter === null) throw new Error("Voplay retained 3D has no WebGPU adapter");
  ...
  private constructor(
    ...
    device.queue.writeBuffer(
      this.#fallbackMaterialParameters,
      0,
      new Float32Array([0, 1, 0, 0, 0, 0.8, 0, 0, 0, 0, 0, 0]),
    );
  ...
  async render(objects: Iterable<RetainedGpuObject>, assets: Iterable<RetainedGpuAsset>): Promise<void> {
    ...
    if (validate) await this.#draw(1, true);
  ...
  #prepareScene(): void {
    ...
    this.#preparedBatches.sort((left, right) =>
      (this.#materials.get(left.material)?.alphaMode === 3 ? 1 : 0)
  ...
  close(): void {
    ...
    this.#meshes.clear();
    for (const material of this.#materials.values()) material.parameters.destroy();
    this.#materials.clear();
    for (const batch of this.#instanceBatches.values()) batch.buffer.destroy();
  ...
  #materialBindGroup(material: ResidentMaterial | undefined): unknown {
    ...
    if (
      ...
      && textureRevisions.every((revision, index) =>
        material.boundTextureRevisions[index] === revision)
    ) {
  ...
  #instanceBuffer(key: string, count: number): GpuBufferLike {
    const current = this.#instanceBatches.get(key);
    if (current !== undefined && current.capacity >= count) return current.buffer;
  ...
  #ensureOverlayCapacity(count: number): void {
    if (this.#overlayCapacity >= count) return;
...
function createSkyPipeline(device: GpuDeviceLike, format: string): GpuPipelineLike {
  const module = device.createShaderModule({
...
function createShadowPipeline(device: GpuDeviceLike): GpuPipelineLike {
  const module = device.createShaderModule({
...
function createOverlayPipeline(device: GpuDeviceLike, format: string): GpuPipelineLike {
  const module = device.createShaderModule({
...
function particleRandom(seed: bigint, index: number, channel: number): number {
  let value = Number(
    (seed + BigInt(index * 1103515245 + channel * 2654435761)) & 0xffffffffn,
  ) >>> 0;
  value ^= value << 13;
  value ^= value >>> 17;
  value ^= value << 5;
...
function decodeMaterial(
  ...
  const textures = Array.from(
    { length: 5 },
    (_, index) => view.getBigUint64(51 + index * 8, true),
  );
...
function sceneViewProjection(camera: SceneCamera, aspect: number): Float32Array {
  ...
  const right = [matrix[0]! / 1000, matrix[4]! / 1000, matrix[8]! / 1000] as const;
  const up = [matrix[1]! / 1000, matrix[5]! / 1000, matrix[9]! / 1000] as const;
  const back = [matrix[2]! / 1000, matrix[6]! / 1000, matrix[10]! / 1000] as const;
  const position = [matrix[3]! / 1000, matrix[7]! / 1000, matrix[11]! / 1000] as const;
...
function decodeOverlays(
  overlays: readonly Uint8Array[],
  width: number,
  height: number,
): number[] {
  ...
  for (const bytes of overlays) {
    ...
    if (bytes[1] === 0x48 && bytes.byteLength >= 81) {
      ...
      if (fill[3] > 0) {
        if (kind === 2) {
          pushOverlayPolygon(
            ...
            width,
            height,
            overlayRoundedRectPoints(x, y, shapeWidth, shapeHeight, radius),
            fill,
          );
        } else if (kind === 3) {
          pushOverlayPolygon(
            ...
            width,
            height,
            overlayEllipsePoints(x, y, shapeWidth, shapeHeight),
            fill,
          );
      ...
      if (stroke[3] > 0 && strokeWidth > 0) {
        if (kind === 2) {
          ...
          const inner = overlayRoundedRectPoints(
            x + inset,
            y + inset,
            Math.max(0.01, shapeWidth - inset * 2),
            Math.max(0.01, shapeHeight - inset * 2),
          ...
          const inset = Math.min(strokeWidth, Math.min(shapeWidth, shapeHeight) * 0.49);
          const inner = overlayEllipsePoints(
            x + inset,
            y + inset,
            Math.max(0.01, shapeWidth - inset * 2),
            Math.max(0.01, shapeHeight - inset * 2),
    ...
    if (bytes[1] === 0x54 && bytes.byteLength >= 72) {
      ...
      if (length > bytes.byteLength - 72) continue;
...
function overlayRoundedRectPoints(
  x: number,
  y: number,
  width: number,
  height: number,
...
function pushOverlayRing(
  ...
  if (outer.length < 3 || outer.length !== inner.length) return;
...
function pushOverlayPolyline(
  ...
  for (let index = 0; index + 1 < points.length; index += 1) {
    ...
    const a = [start[0] + normalX, start[1] + normalY] as const;
    const b = [end[0] + normalX, end[1] + normalY] as const;
    const c = [end[0] - normalX, end[1] - normalY] as const;
    const d = [start[0] - normalX, start[1] - normalY] as const;
...
function pushOverlayRectStroke(
  output: number[],
  width: number,
  height: number,
  x: number,
...
function pushOverlayScreenTriangle(
  output: number[],
  width: number,
  height: number,
...
function isOverlay(bytes: Uint8Array): boolean {
