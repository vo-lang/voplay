...
type VoplayPerfMode = "off" | "stats" | "hud" | "trace" | "deep";
type DisplayPulseMode = "hybrid" | "raf" | "timer";

type PerfWindowStats = {
  count: number;
  p50: number;
  p90: number;
  p99: number;
...
type VoplayDebugGlobal = typeof globalThis & {
  ...
  __voplayWebGpuPerfProbe?: WebGpuPerfProbe;
  __voplayWebGpuPerfReport?: (message: string) => void;
  __voplayWebGpuPerfPacket?: Uint8Array;
  __voplayTakeWebGpuPerfPacket?: () => Uint8Array;
  __voplayRendererPerfConfig?: () => string;
  __voplayPerfReports?: VoplayPerfReportPayload[];
};

type VoplayPerfReportPayload = {
  schemaVersion: number;
  source: "voplay-render-bootstrap";
  kind: "bootstrap" | "pulse" | "webgpu" | "renderer" | "liveness";
  mode: VoplayPerfMode;
  message: string;
...
type WebGpuQueueInstance = {
  onSubmittedWorkDone?: () => Promise<unknown>;
};

type WebGpuQueuePrototype = {
  submit?: (this: WebGpuQueueInstance, ...args: unknown[]) => unknown;
  __voplayOriginalSubmit?: (this: WebGpuQueueInstance, ...args: unknown[]) => unknown;
};

type WebGpuCanvasContextPrototype = {
  getCurrentTexture?: (this: unknown, ...args: unknown[]) => unknown;
  __voplayOriginalGetCurrentTexture?: (this: unknown, ...args: unknown[]) => unknown;
};

type WebGpuPerfPacketMetrics = {
  acquire: PerfWindowStats;
  submitCpu: PerfWindowStats;
  workDone: PerfWindowStats;
  workDoneRaw?: PerfWindowStats;
  probeCpu: PerfWindowStats;
  queueDepthClass: "empty" | "normal" | "backlogged" | "saturated";
  sampleRate: number;
  workDoneInFlight: number;
  workDoneErrors: number;
  reportOverheadMs: number;
};

installWebGpuPacketBridge(globalThis as VoplayDebugGlobal);
installRendererPerfConfigBridge(globalThis as VoplayDebugGlobal);

const perfReportPostQueue: VoplayPerfReportPayload[] = [];
let perfReportPostTimer: number | null = null;

class WebGpuPerfProbe {
  private installed = false;
  private submitCount = 0;
  private submitCpuWindow: number[] = [];
  private textureAcquireWindow: number[] = [];
  private workDoneWindow: number[] = [];
  private probeCpuWindow: number[] = [];
  private workDoneInFlight = 0;
  private workDoneErrors = 0;
  private workDoneHostSuspendedSamples = 0;
  private lastSubmitNowMs: number | null = null;
  private submitGapGeneration = 0;

  install(globalScope: VoplayDebugGlobal): void {
    if (this.installed) return;
    this.installed = true;
    const globals = globalScope as unknown as Record<string, unknown>;
    const queueCtor = globals.GPUQueue as { prototype?: WebGpuQueuePrototype } | undefined;
    ...
    if (queueProto?.submit && !queueProto.__voplayOriginalSubmit) {
      ...
      queueProto.submit = function (this: WebGpuQueueInstance, ...args: unknown[]): unknown {
    ...
    const canvasContextCtor = globals.GPUCanvasContext as { prototype?: WebGpuCanvasContextPrototype } | undefined;
    ...
    if (canvasContextProto?.getCurrentTexture && !canvasContextProto.__voplayOriginalGetCurrentTexture) {
      ...
      canvasContextProto.getCurrentTexture = function (this: unknown, ...args: unknown[]): unknown {
        const startMs = performance.now();
        try {
  ...
  recordTextureAcquire(durationMs: number): void {
    pushWindowSample(this.textureAcquireWindow, durationMs, PERF_SAMPLE_WINDOW);
  }

  recordProbeOverhead(durationMs: number): void {
    pushWindowSample(this.probeCpuWindow, durationMs, PERF_SAMPLE_WINDOW);
  }

  recordSubmit(queue: WebGpuQueueInstance, cpuMs: number): void {
    this.submitCount += 1;
  ...
  private sampleSubmittedWorkDone(queue: WebGpuQueueInstance): void {
  ...
  private reportWindow(): void {
    ...
    window.setTimeout(() => {
      this.emitWebGpuPerfWindow(
        textureAcquireSamples,
        submitCpuSamples,
        workDoneSamples,
        probeCpuSamples,
        workDoneInFlight,
  ...
  private emitWebGpuPerfWindow(
    textureAcquireSamples: number[],
    submitCpuSamples: number[],
    workDoneSamples: number[],
    probeCpuSamples: number[],
    workDoneInFlight: number,
    workDoneErrors: number,
    workDoneHostSuspendedSamples: number,
  ): void {
...
function installWebGpuPacketBridge(globalScope: VoplayDebugGlobal): void {
  if (globalScope.__voplayTakeWebGpuPerfPacket) return;
...
function installRendererPerfConfigBridge(globalScope: VoplayDebugGlobal): void {
  if (globalScope.__voplayRendererPerfConfig) return;
...
function readRendererPerfConfig(): string {
  const tokens: string[] = [];
...
function appendRendererPerfConfigTokens(tokens: string[], raw: string | null | undefined): void {
  if (!raw) return;
...
function mergeRendererPerfConfigs(...configs: string[]): string {
  const tokens: string[] = [];
...
function appendRendererPerfSwitch(tokens: string[], token: string, paramNames: string[], storageKey: string): void {
  const value = readRendererPerfSwitch(paramNames, storageKey);
  if (value) tokens.push(token);
}

function readRendererPerfSwitch(paramNames: string[], storageKey: string): boolean {
  for (const name of paramNames) {
    const raw = readLocationParam(name);
    if (raw !== null) return readTruthyPerfValue(raw);
  }
  try {
    const raw = globalThis.localStorage?.getItem(storageKey);
    if (raw !== null && raw !== undefined) return readTruthyPerfValue(raw);
  } catch {
    return false;
  }
  return false;
...
function readTruthyPerfValue(raw: string): boolean {
  const value = raw.trim().toLowerCase();
  return value !== "" && value !== "0" && value !== "false" && value !== "off" && value !== "no";
}

function encodeWebGpuPerfPacket(metrics: WebGpuPerfPacketMetrics): Uint8Array {
  const payloadSize = 4 + 4 * 4 + 6 * 8;
  const data = new Uint8Array(PERF_PACKET_HEADER_SIZE + payloadSize);
  const view = new DataView(data.buffer);
  let offset = 0;
  view.setUint8(offset, PERF_PACKET_MAGIC); offset += 1;
  view.setUint8(offset, PERF_PACKET_VERSION); offset += 1;
  writeU32(view, offset, PERF_PACKET_SCHEMA_VERSION); offset += 4;
  writeU32(view, offset, 0); offset += 4;
  writeU32(view, offset, metrics.submitCpu.count); offset += 4;
  writeU32(view, offset, PERF_PACKET_SOURCE_WEBGPU); offset += 4;
  writeU32(view, offset, payloadSize); offset += 4;
  writeF64(view, offset, metrics.submitCpu.p90); offset += 8;
  writeF64(view, offset, metrics.workDone.p90); offset += 8;
  writeU32(view, offset, metrics.workDoneInFlight); offset += 4;
  writeU32(view, offset, 0); offset += 4;
  writeU32(view, offset, 1); offset += 4;

  writeU32(view, offset, WEBGPU_PERF_PAYLOAD_VERSION); offset += 4;
  writeU32(view, offset, webGpuQueueDepthClassCode(metrics.queueDepthClass)); offset += 4;
  writeU32(view, offset, metrics.submitCpu.count); offset += 4;
  writeU32(view, offset, WEBGPU_WORK_DONE_SAMPLE_STRIDE); offset += 4;
  writeU32(view, offset, metrics.workDoneErrors); offset += 4;
  writeF64(view, offset, metrics.acquire.p90); offset += 8;
  writeF64(view, offset, metrics.submitCpu.p90); offset += 8;
  writeF64(view, offset, metrics.workDone.p90); offset += 8;
  writeF64(view, offset, metrics.sampleRate); offset += 8;
  writeF64(view, offset, metrics.probeCpu.p99); offset += 8;
...
function webGpuQueueDepthClassCode(value: WebGpuPerfPacketMetrics["queueDepthClass"]): number {
  if (value === "empty") return 0;
  if (value === "backlogged") return 2;
  if (value === "saturated") return 3;
  return 1;
}

function pushWindowSample(samples: number[], value: number, capacity: number): void {
  samples.push(value);
...
function measureSamples(samples: number[]): PerfWindowStats {
  if (samples.length === 0) {
...
function countAboveSamples(samples: number[], threshold: number): number {
  let count = 0;
...
function classifyQueueDepth(workDone: PerfWindowStats, inFlight: number): "empty" | "normal" | "backlogged" | "saturated" {
  if (workDone.count === 0 && inFlight === 0) return "empty";
  if (inFlight >= 4 || workDone.p90 >= 100) return "saturated";
  if (inFlight >= 2 || workDone.p90 >= FRAME_BUDGET_120_MS * 4) return "backlogged";
  return "normal";
}

function isHostSuspendedWorkDoneSample(
  elapsedMs: number,
  submitsWhilePending: number,
  submitGapGenerationAtSample: number,
  currentSubmitGapGeneration: number,
): boolean {
  if (elapsedMs < WEBGPU_WORK_DONE_HOST_SUSPEND_MS) return false;
...
function sendVoplayPerfReport(kind: VoplayPerfReportPayload["kind"], message: string, metrics: Record<string, unknown>): void {
  const mode = readVoplayPerfMode();
  const payload: VoplayPerfReportPayload = {
...
function queueVoplayPerfReportPost(payload: VoplayPerfReportPayload): void {
  ...
  if (perfReportPostTimer !== null) return;
  try {
...
function flushVoplayPerfReportPosts(): void {
...
function postVoplayPerfReports(payloads: VoplayPerfReportPayload[]): void {
  let body = "";
  try {
...
function locationHasParam(name: string): boolean {
  return readLocationParam(name) !== null;
}

function readVoplayPerfMode(): VoplayPerfMode {
  const raw =
    readLocationParam("voplayPerf") ??
    readLocationParam("perf") ??
    globalThis.localStorage?.getItem("voplay.perf.mode") ??
    (shouldDefaultRunnerPerfStats() ? "stats" : null) ??
    "off";
  if (raw === "stats" || raw === "hud" || raw === "trace" || raw === "deep") return raw;
  if (raw === "1" || raw === "true") return "stats";
  return "off";
}

function shouldDefaultRunnerPerfStats(): boolean {
  try {
    const location = globalThis.location;
    if (!location || !isLocalRunnerHost(location.hostname)) return false;
...
function isLocalRunnerHost(hostname: string): boolean {
  return hostname === "localhost" || hostname === "127.0.0.1" || hostname === "::1" || hostname === "[::1]";
}

function isVoplayPerfEnabled(mode: VoplayPerfMode): boolean {
  return mode !== "off";
}

function shouldEmitVerbosePerfReports(mode = readVoplayPerfMode()): boolean {
  return mode === "trace" || mode === "deep";
}

function shouldPostPerfReports(mode = readVoplayPerfMode()): boolean {
  return isVoplayPerfEnabled(mode) || locationHasParam("voplayPerf") || locationHasParam("perf") || locationHasParam("voplayPerfOverlay");
}

function shouldLogPerfReports(mode = readVoplayPerfMode()): boolean {
  return shouldEmitVerbosePerfReports(mode) || locationHasParam("voplayPerfOverlay");
}

function shouldSampleWebGpuWorkDone(mode = readVoplayPerfMode()): boolean {
  try {
    return mode === "deep"
      || mode === "trace"
...
function shouldInstallWebGpuPerfProbe(mode = readVoplayPerfMode()): boolean {
  return isVoplayPerfEnabled(mode) || shouldSampleWebGpuWorkDone(mode);
}

function shouldShowBootstrapPerfOverlay(mode: VoplayPerfMode): boolean {
  try {
    if (locationHasParam("voplayPerfOverlay")) return true;
    if (globalThis.localStorage?.getItem("voplay.perf.overlay") === "1") return true;
    if (shouldCaptureRendererDebugStatus()) return true;
  } catch {
    return false;
  }
  return mode === "deep";
...
function shouldCaptureRendererDebugStatus(): boolean {
  try {
...
function readDisplayPulseMode(): DisplayPulseMode {
  try {
    const raw = [
    ...
    for (const token of raw.split(/[,\s;&|]+/)) {
      ...
      if (mode === "raf" || mode === "pulseraf") return "raf";
      if (mode === "timer" || mode === "pulsetimer") return "timer";
      if (mode === "hybrid" || mode === "pulsehybrid") return "hybrid";
    }
  } catch {
    return "hybrid";
  }
...
export class RenderIsland {
  ...
  private perfMode: VoplayPerfMode = readVoplayPerfMode();
  private displayPulseMode: DisplayPulseMode = readDisplayPulseMode();
  ...
  private wakeCount = 0;
  private displayPulseRafId: number | null = null;
  private displayPulseTimerId: number | null = null;
  private displayPulseScheduleId = 0;
  private displayPulseSerial = 0;
  private displayPulseLastWakeMs: number | null = null;
  private displayPulseLastRafMs: number | null = null;
  private displayPulseTimerLeadMs = 0;
  private displayPulseHealthyRafFrames = 0;
  private displayPulseSlowRafFrames = 0;
  private displayPulseTimerCadence = true;
  private displayPulseWaiters = new Map<string, DisplayPulseWaiter>();
  private displayPulseWaitWindow: number[] = [];
  private displayPulseRafWakeWindow = 0;
  private displayPulseTimerWakeWindow = 0;
  private displayPulseLastSource: "raf" | "timer" | null = null;
  private vmWakeRunWindow: number[] = [];
  private scheduledHostEventCount = 0;
  private scheduledDisplayPulseCount = 0;
  private scheduledTimeoutCount = 0;
  private lastHostEventDelayMs: number | null = null;
  private perfLivenessTimerIds: number[] = [];
  private pulseOverlay: HTMLElement | null = null;
  private pulseMessage = "";
  private webGpuMessage = "";
  private rendererDebugMessage = "";
  private webGpuReportCallback: ((message: string) => void) | null = null;
  private rendererDebugStatusCallback: ((message: string) => void) | null = null;
  ...
  private reportBootstrapStart(): void {
    if (!this.perfEnabled() || !shouldPostPerfReports(this.perfMode)) return;
  ...
  private schedulePerfLivenessProbes(): void {
    if (!this.perfEnabled() || !shouldPostPerfReports(this.perfMode)) return;
  ...
  private collectPerfLivenessMetrics(): Record<string, unknown> {
    return {
  ...
  private ensureDisplayPulseTicker(): void {
    if (this.displayPulseRafId !== null || this.displayPulseTimerId !== null || this.stopped || !this.vm) return;
  ...
  private displayPulseCadenceDelayMs(nowMs: number): number {
    if (this.displayPulseLastWakeMs === null) return DISPLAY_PULSE_TIMER_TARGET_MS;
  ...
  private clearDisplayPulseSchedule(): void {
  ...
  private handleDisplayPulse(scheduleId: number, source: "raf" | "timer"): void {
    if (scheduleId !== this.displayPulseScheduleId) return;
    const nowMs = performance.now();
  ...
  private handleDisplayPulseTick(source: "raf" | "timer", nowMs: number): void {
    if (this.stopped || !this.vm) return;
  ...
  private recordRafPulse(nowMs: number): void {
    if (this.displayPulseLastRafMs !== null) {
      ...
      if (deltaMs <= DISPLAY_PULSE_RAF_HEALTHY_MS) {
        ...
        if (
          !DISPLAY_PULSE_HYBRID_KEEP_TIMER_CADENCE
  ...
  private wakeHostEvent(token: string, delayMs: number): void {
    try {
      ...
      this.wakeCount += 1;
      this.debugFrameStatus(
        `render island wake #${this.wakeCount} delay=${delayMs}`,
        this.wakeCount,
      );
  ...
  private recordDisplayPulseWait(waitMs: number): void {
  ...
  private recordVmWakeRun(runMs: number): void {
    if (!this.perfEnabled()) return;
  ...
  private reportDisplayPulseWindow(): void {
  ...
  private emitDisplayPulseWindow(
    ...
    const message =
      `render island pulse window samples=${pulse.count}` +
  ...
  private showPerfOverlay(): void {
    if (!shouldShowBootstrapPerfOverlay(this.perfMode)) return;
  ...
  private measureWindow(samples: number[]): PerfWindowStats {
    if (samples.length === 0) {
      return { count: 0, p50: 0, p90: 0, p99: 0, max: 0 };
    }
    const sorted = [...samples].sort((a, b) => a - b);
  ...
  private countAbove(samples: number[], threshold: number): number {
    let count = 0;
    for (const sample of samples) {
      if (sample > threshold) count += 1;
    }
  ...
  private installWebGpuPerfProbe(): void {
    if (!this.perfEnabled() || !shouldInstallWebGpuPerfProbe(this.perfMode)) return;
    const globalScope = globalThis as VoplayDebugGlobal;
    this.webGpuReportCallback = (message: string) => {
      if (this.stopped) return;
  ...
  private installRendererDebugStatusProbe(): void {
    if (!shouldCaptureRendererDebugStatus()) return;
    const globalScope = globalThis as VoplayDebugGlobal;
    this.rendererDebugStatusCallback = (message: string) => {
      if (this.stopped) return;
