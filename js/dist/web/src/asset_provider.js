...
export class VoplayBrowserAssetProvider {
    ...
    async init(host) {
        if (this.#host !== null)
            throw new Error("Voplay asset provider already initialized");
        ...
        if (lanes === null)
            throw new Error("Voplay asset provider requires framework_lane");
        const assetBuffers = host.getCapability("asset_buffer");
        if (assetBuffers === null)
            throw new Error("Voplay asset provider requires asset_buffer");
    ...
    ownerSnapshot() {
        ...
        for (const ticket of this.#tickets.values()) {
            if (ticket.state === "pending")
                pendingTickets += 1;
            else if (ticket.state === "dispatched")
                dispatchedTickets += 1;
            else
                terminalTickets += 1;
        }
    ...
    async #dispatch(packet) {
        ...
        switch (header.kind) {
            case 12 /* MessageKind.EngineStart */:
                ...
                return;
            case 21 /* MessageKind.AssetRequest */:
                this.#requireState(header.engine, "running");
                await this.#request(packet);
                return;
            case 23 /* MessageKind.AssetControl */:
            ...
            case 32 /* MessageKind.WorkerWake */:
                ...
                if (payload.byteLength !== 0)
                    throw new Error("asset wake payload must be empty");
                await this.#flushWork(packet);
                return;
            case 14 /* MessageKind.EngineSuspend */:
                this.#requireState(header.engine, "running");
                if (payload.byteLength !== 0)
                    throw new Error("asset suspend payload must be empty");
                this.#state = "suspended";
                return;
            case 15 /* MessageKind.EngineResume */:
                this.#requireState(header.engine, "suspended");
                if (payload.byteLength !== 0)
                    throw new Error("asset resume payload must be empty");
                this.#state = "running";
                await this.#flushWork(packet);
                return;
            case 16 /* MessageKind.EngineClose */:
                ...
                if (payload.byteLength !== 0)
                    throw new Error("asset close payload must be empty");
                for (const ticket of this.#tickets.values()) {
                    if (ticket.state !== "terminal")
                        await this.#finishTicket(packet, ticket, 4);
                }
                this.#state = "closed";
    ...
    async #request(packet) {
        ...
        if (this.#tickets.size >= MAX_TICKETS)
            throw new Error("Voplay asset ticket capacity exceeded");
        const ticket = {
        ...
        this.#touch();
        await this.#reply(packet, 22 /* MessageKind.AssetCompletion */, concatBytes(Uint8Array.of(2), encodeHandle(ticket.handle), encodeHandle(assetRef)));
        await this.#flushWork(packet);
    }
        ...
        switch (tag) {
            ...
            case 3: {
                ...
                if (this.#scopes.size >= MAX_SCOPES)
                    throw new Error("Voplay asset scope capacity exceeded");
                const scope = {
                ...
                this.#touch();
                await this.#reply(packet, 22 /* MessageKind.AssetCompletion */, concatBytes(Uint8Array.of(1), encodeHandle(scope.handle), Uint8Array.of(scope.kind)));
                return;
            }
            case 4:
            ...
            case 10:
                if (body.byteLength !== 0)
                    throw new Error("invalid Voplay asset restart");
    ...
    async #register(packet, registrations, hotReload) {
        if (hotReload && registrations.length !== 1)
            throw new Error("invalid asset hot reload batch");
        ...
        if (assetBuffers === null)
            throw new Error("Voplay asset buffer capability is unavailable");
        const refs = [];
        const prepared = [];
        for (const registration of registrations) {
            const key = bytesKey(registration.assetId);
            ...
            if (hotReload) {
                if (!current)
                    throw new Error("Voplay asset hot reload preflight diverged");
        ...
        try {
            ...
            for (const record of attempted.reverse()) {
                ...
                try {
                    ...
                    this.#host?.reportError(`Voplay asset binding rollback failed: ${errorMessage(rollbackError)}`);
                }
            }
            throw error;
        }
        ...
        await this.#reply(packet, 22 /* MessageKind.AssetCompletion */, concatBytes(Uint8Array.of(5), count, ...refs.map(encodeHandle)));
        if (hotReload)
            await this.#flushWork(packet);
    }
    async #terminalControl(packet, body, outcome, allowMissing) {
        if (body.byteLength !== 8)
            throw new Error("invalid Voplay asset ticket handle");
        ...
        if (ticket.state !== "terminal")
            await this.#finishTicket(packet, ticket, outcome);
    }
    async #closeScope(packet, body) {
        if (body.byteLength !== 8)
            throw new Error("invalid Voplay asset scope handle");
        const scope = this.#scopes.get(handleKey(readHandle(body, 0)));
        if (!scope || !scope.open)
            throw new Error("unknown or closed Voplay asset scope");
        scope.open = false;
        this.#touch();
        for (const ticket of this.#tickets.values()) {
    ...
    #releaseTicket(body) {
        if (body.byteLength !== 8)
            throw new Error("invalid Voplay asset ticket release");
    ...
    #releaseScope(body) {
        if (body.byteLength !== 8)
            throw new Error("invalid Voplay asset scope release");
        const key = handleKey(readHandle(body, 0));
        const scope = this.#scopes.get(key);
        if (!scope || scope.open)
            throw new Error("Voplay asset scope must be closed before release");
        for (const ticket of this.#tickets.values()) {
    ...
    async #expire(packet, body) {
        if (body.byteLength !== 8)
            throw new Error("invalid Voplay asset expiry");
    ...
    async #terminalizeWork(packet, body, outcome) {
        if (body.byteLength !== ASSET_WORK_BYTES)
            throw new Error("invalid Voplay asset work result");
        const ticket = this.#tickets.get(handleKey(readHandle(body, 0)));
        const assetRef = readHandle(body, 8);
        const resultEndpoint = readHandle(body, 56);
        if (ticket === undefined || ticket.state === "terminal")
            return;
        if (!sameHandle(ticket.assetRef, assetRef)
        ...
        if (resultRevision < asset.sourceRevision)
            return;
        if (resultRevision > asset.sourceRevision
    ...
    async #flushWork(packet) {
        ...
        for (const ticket of this.#tickets.values()) {
            ...
            if (!asset)
                throw new Error("Voplay asset ticket lost its asset");
            ...
            await this.#reply(packet, 22 /* MessageKind.AssetCompletion */, concatBytes(Uint8Array.of(3), encodeHandle(ticket.handle), encodeHandle(asset.assetRef), asset.assetId, sourceRevision, asset.artifactId, encodeHandle(this.#endpointGeneration)));
        }
    }
    async #finishTicket(packet, ticket, outcome) {
        ticket.state = "terminal";
        this.#touch();
        await this.#reply(packet, 22 /* MessageKind.AssetCompletion */, concatBytes(Uint8Array.of(4), encodeHandle(ticket.handle), encodeHandle(ticket.assetRef), Uint8Array.of(outcome)));
    }
    async #controlAck(packet, command) {
        await this.#reply(packet, 22 /* MessageKind.AssetCompletion */, Uint8Array.of(6, command));
    }
    #validateEnvelope(packet) {
        const lane = this.#lane;
        if (lane === null)
            throw new Error("Voplay asset lane is closed");
    ...
    async #reply(packet, kind, payload) {
        const lane = this.#lane;
        if (lane === null)
            throw new Error("Voplay asset lane is closed");
    ...
    #touch() {
        if (this.#revision >= 0xffffffffffffffffn) {
...
function decodeRegistrations(bytes, counted) {
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const count = counted ? requireU32(view, 0) : 1;
    if (count === 0 || count > MAX_ASSETS)
        throw new Error("invalid Voplay asset registration count");
    let offset = counted ? 4 : 0;
    let dependencyCount = 0;
    const registrations = [];
    const identities = new Set();
    for (let index = 0; index < count; index += 1) {
        if (bytes.byteLength - offset < 52)
            throw new Error("truncated Voplay asset registration");
        const assetId = bytes.slice(offset, offset + 16);
        const key = bytesKey(assetId);
        if (isZero(assetId) || identities.has(key))
            throw new Error("invalid or duplicate Voplay AssetId");
        identities.add(key);
        ...
        for (let dependency = 0; dependency < dependencies; dependency += 1) {
            ...
            if (isZero(id))
                throw new Error("invalid Voplay dependency AssetId");
    ...
    if (offset !== bytes.byteLength)
        throw new Error("trailing Voplay asset registration bytes");
    return registrations;
}
function allocateHandle(index) {
...
function readHandle(bytes, offset) {
    if (bytes.byteLength - offset < 8)
        throw new Error("truncated Voplay handle");
    ...
    if (handle.index === 0xffffffff || handle.generation === 0)
        throw new Error("invalid Voplay handle");
    return handle;
}
    ...
    for (const [asset, dependencies] of graph) {
        ...
        for (const dependency of dependencies) {
            if (!graph.has(dependency))
                throw new Error("unknown Voplay asset dependency");
        }
    }
    const active = new Set();
    const complete = new Set();
    const visit = (asset) => {
        if (complete.has(asset))
            return;
        if (active.has(asset))
            throw new Error("Voplay asset dependency cycle");
        active.add(asset);
