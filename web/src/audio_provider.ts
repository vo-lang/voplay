...
interface StudioProviderHost {
  ...
  getCapability(name: "app_surface"): {
    isInteractive(): boolean;
    subscribeInput(sink: (event: {
...
interface BrowserAudioBus {
  readonly handle: GenerationalHandle;
  readonly parent: GenerationalHandle | null;
  readonly gain: number;
  readonly mute: boolean;
  readonly solo: boolean;
  readonly listener: BrowserAudioListener | null;
  readonly ducking: readonly BrowserAudioDuckingRule[];
}

interface BrowserAudioListener {
  readonly position: readonly [number, number, number];
  readonly right: readonly [number, number, number];
}

interface BrowserAudioDuckingRule {
  readonly trigger: GenerationalHandle;
  readonly target: GenerationalHandle;
  readonly gain: number;
}

interface BrowserPersistentSource {
  readonly handle: GenerationalHandle;
  readonly bus: GenerationalHandle;
  readonly asset: GenerationalHandle;
  readonly gain: number;
  readonly loop: boolean;
  readonly recovery: 1 | 2 | 3;
  readonly transportAnchorMillis: bigint;
  readonly spatial: BrowserSpatialSource | null;
}

interface BrowserSpatialSource {
  readonly position: readonly [number, number, number];
  readonly minDistance: number;
  readonly maxDistance: number;
}

interface BrowserAudioControl {
  readonly buses: BrowserAudioBus[];
  readonly sources: BrowserPersistentSource[];
  readonly retiringBuses: BrowserRetiringAudioBus[];
  readonly retiringSources: BrowserRetiringAudioSource[];
  readonly entries: BrowserAudioControlEntry[];
}

interface BrowserAudioControlEntry {
  readonly kind: number;
  readonly handle: GenerationalHandle;
  readonly state: 1 | 2;
  readonly descriptor: Uint8Array;
  readonly fence: BrowserRetirementFence;
}

interface BrowserRetirementFence {
  readonly renderRevision: bigint;
  readonly domainFrame: bigint;
  readonly eventSequence: bigint;
  readonly audioEventSequence: bigint;
}

interface BrowserRetiringAudioBus {
  readonly value: BrowserAudioBus;
  readonly fence: BrowserRetirementFence;
}

interface BrowserRetiringAudioSource {
  readonly value: BrowserPersistentSource;
  readonly fence: BrowserRetirementFence;
}

type AudioProviderState = "created" | "running" | "closed";
type AudioDeviceState = "ready-locked" | "active" | "suspended" | "lost";

interface OwnedAudioSource {
...
interface OwnedPersistentAudioSource extends OwnedAudioSource {
...
const AUDIO_CONTROL_MAGIC = [0x56, 0x41, 0x43, 0x53] as const;
...
export class VoplayBrowserAudioProvider {
  ...
  #polling = false;
  #state: AudioProviderState = "created";
  ...
  #nowMillis = 0n;
  #audioFrameOffset = 0n;
  #lastControl: FrameworkPacket | null = null;
  #lastObservedProgress = "";
  #publishedObservations = new Set<string>();
  #realizationFailures = new Set<string>();
  #observedAudioEventSequence = 0n;
  #deviceState: AudioDeviceState = "ready-locked";
  #audioContext: AudioContext | null = null;
  #assetBuffers: BrowserAssetBufferCapability | null = null;
  #control: BrowserAudioControl = {
    buses: [], sources: [], retiringBuses: [], retiringSources: [], entries: [],
  };
  #busNodes = new Map<string, GainNode>();
  #persistentNodes = new Map<string, OwnedPersistentAudioSource>();
  #persistentStarts = new Map<string, bigint>();
  #recoveryOffsetsMillis = new Map<string, bigint>();
  #completedPersistent = new Set<string>();
  #nextPersistentStart = 1n;
  #oneShotNodes = new Map<bigint, OwnedAudioSource>();
  #nextOneShot = 1n;
  #decodedAssets = new Map<string, DecodedAudioAsset>();
  #decodedAudioBytes = 0;
  #decodedUseClock = 1n;
  #audioAssets = new Map<string, { readonly revision: bigint; readonly bytes: Uint8Array }>();
  #audioAssetRevisions = new Map<string, bigint>();
  #audioAssetTombstones = new Set<string>();
  #audioAssetBytes = 0;
  ...
  async init(host: StudioProviderHost): Promise<void> {
    if (this.#host !== null) throw new Error("Voplay audio provider already initialized");
    ...
    if (lanes === null) throw new Error("Voplay audio provider requires framework_lane");
    ...
    this.#unsubscribeInput = appSurface?.subscribeInput((event) => {
      if (
        (this.#deviceState === "ready-locked" || this.#deviceState === "suspended")
  ...
  quiesceForCapture(): { stopped: number; state: AudioProviderState } {
  ...
  ownerSnapshot(): {
    ...
    readonly persistentSources: number;
    readonly completedPersistentSources: number;
    readonly oneShots: number;
    readonly decodedAssets: number;
    readonly decodedAudioBytes: number;
    readonly audioAssets: number;
    readonly audioAssetBytes: number;
    readonly activationAttempts: bigint;
    readonly deviceLosses: bigint;
    readonly deviceRecoveries: bigint;
    readonly oneShotsStarted: bigint;
    readonly oneShotsFinished: bigint;
    readonly observationHistory: number;
  } {
    return {
      state: this.#state,
  ...
  async #dispatch(packet: FrameworkPacket): Promise<void> {
    ...
    switch (header.kind) {
      ...
      case MessageKind.AudioControlTransaction:
        ...
        if (this.#audioContext !== null) this.#capturePersistentRecoveryOffsets();
        ...
        for (const key of this.#recoveryOffsetsMillis.keys()) {
          if (!retainedSourceKeys.has(key)) this.#recoveryOffsetsMillis.delete(key);
        }
        for (const key of this.#completedPersistent) {
          if (!retainedSourceKeys.has(key)) this.#completedPersistent.delete(key);
        }
      ...
      case MessageKind.AudioEvent:
        ...
        if (header.newRevision <= this.#nowMillis) {
          ...
          if (!this.#control.buses.some((bus) => sameHandle(bus.handle, event.bus))) {
            ...
            try {
              const played = await this.#playOneShot(
                event,
              ...
              this.#host?.log(
                `Voplay one-shot audio event failed: ${errorMessage(error)}`,
              );
            }
          }
        }
        await this.#reply(packet, MessageKind.AudioEventResult, Uint8Array.of(outcome), {
        ...
        await this.#publishControlObservation();
        return;
      case MessageKind.ControlObservedAck:
        this.#requireRunning(header.engine);
        this.#acceptControlObservedAck(packet);
        return;
      case MessageKind.DeviceEvent: {
        this.#requireRunning(header.engine);
        if (payload.byteLength < 1) throw new Error("truncated Voplay audio DeviceEvent");
        const tag = payload[0];
        if (tag === 1 && (payload.byteLength === 17 || payload.byteLength === 75)) {
          ...
          if (
            !sameHandle(endpoint, this.#endpointGeneration)
            || !validGenerationalHandle(gesture)
          ) {
            throw new Error("invalid Voplay browser audio gesture identity");
          }
          if (payload.byteLength === 75) validateBrowserAudioPermit(payload.subarray(17));
          await this.#unlock();
        } else if (tag === 2 && payload.byteLength === 1) {
          if (this.#deviceState !== "lost") this.#deviceLosses += 1n;
          this.#deviceState = "lost";
          this.#capturePersistentRecoveryOffsets();
          ...
          if (payload.byteLength === 59) validateBrowserAudioPermit(payload.subarray(1));
          this.#deviceState = "ready-locked";
      ...
      case MessageKind.EngineSuspend:
        this.#requireRunning(header.engine);
        if (payload.byteLength !== 0) throw new Error("audio lifecycle payload must be empty");
      ...
      case MessageKind.EngineResume:
        ...
        if (this.#deviceState === "active") await this.#publishControlObservation();
        return;
      ...
      case MessageKind.EngineClose:
        this.#requireRunning(header.engine);
        if (payload.byteLength !== 0) throw new Error("audio close payload must be empty");
        this.#state = "closed";
        this.#deviceState = "lost";
        if (this.#audioContext !== null) {
          this.#audioContext.onstatechange = null;
  ...
  async #unlock(): Promise<void> {
    if (this.#deviceState !== "ready-locked" || this.#host === null) return;
    ...
    try {
      ...
      if (this.#deviceState === "active") {
        ...
        if (recovering) this.#deviceRecoveries += 1n;
        if (this.#lastControl !== null) {
          await this.#publishRealizationResults(this.#lastControl);
        }
        await this.#publishControlObservation();
      ...
      if (context !== null) await this.#discardContext(context, false);
      this.#host?.reportError(`Voplay audio unlock denied: ${errorMessage(error)}`);
    }
  }

  async #activateFromGesture(): Promise<void> {
    if (this.#deviceState === "ready-locked") {
      await this.#unlock();
      return;
    }
    const context = this.#audioContext;
    if (this.#deviceState !== "suspended" || context === null) return;
    try {
      await context.resume();
  ...
  async #discardContext(context: AudioContext, preserveTimeline: boolean): Promise<void> {
    context.onstatechange = null;
    if (this.#audioContext === context) {
      if (preserveTimeline) {
        this.#capturePersistentRecoveryOffsets();
        this.#captureAudioFrameOffset();
        if (this.#state === "running") this.#deviceLosses += 1n;
      }
      this.#disconnectGraph();
      this.#clearDecodedAssets();
      this.#audioContext = null;
  ...
  async #rebuildGraph(
    ...
    if (context === null) return failures;
    this.#disconnectGraph();
    const buses = [
      ...this.#control.buses,
    ...
    for (const bus of buses) {
      ...
      if (bus.parent === null) {
        ...
        if (parent === undefined) throw new Error("Voplay audio bus parent is unavailable");
        node.connect(parent);
      }
    }
    const listener = this.#control.buses.find((bus) => bus.listener !== null)?.listener;
    if (listener !== undefined && listener !== null) this.#applyListener(listener);
    ...
    for (const source of sources) {
      try {
        ...
        this.#host?.log(
          `Voplay persistent audio source ${handleKey(source.handle)} awaits realization retry: ${errorMessage(error)}`,
        );
      }
    }
    const physicalSources = new Set(sources.map((source) => handleKey(source.handle)));
    for (const key of this.#recoveryOffsetsMillis.keys()) {
      if (!physicalSources.has(key)) this.#recoveryOffsetsMillis.delete(key);
    }
    return failures;
  }

  async #startPersistent(
    source: BrowserPersistentSource,
    recovering = false,
    preserveTimeline = false,
  ): Promise<void> {
    const key = handleKey(source.handle);
    if (this.#completedPersistent.has(key)) return;
    ...
    if (this.#nextPersistentStart > 0xffff_ffff_ffff_ffffn) {
    ...
    node.onended = () => {
      if (this.#persistentNodes.get(key)?.source === node) {
        ...
        if (!source.loop) this.#completedPersistent.add(key);
        this.#syncBusMixState();
      }
      node.disconnect();
      for (const owned of nodes) owned.disconnect();
    };
    try {
      node.start(0, offset);
    } catch (error) {
    ...
    if (previous !== undefined) {
      ...
      for (const owned of previous.nodes) owned.disconnect();
    }
    this.#completedPersistent.delete(key);
  ...
  async #playOneShot(
    event: ReturnType<typeof decodeOneShot>,
    deadlineBudgetMillis: bigint,
    deadlineStarted: number,
  ): Promise<"played" | "deadline" | "dropped" | "failed"> {
    if (!this.#control.buses.some((bus) => sameHandle(bus.handle, event.bus))) {
      return "failed";
    }
    ...
    if (context === null || bus === undefined) return "failed";
    const buffer = await this.#decodeAsset(event.asset);
    this.#syncInteractiveOutput();
    const elapsedMillis = BigInt(
      Math.max(0, Math.floor(performance.now() - deadlineStarted)),
    );
    if (deadlineBudgetMillis === 0n || elapsedMillis >= deadlineBudgetMillis) {
      return "deadline";
    }
    if (!this.#interactive) return "dropped";
    if (this.#audioContext !== context || this.#deviceState !== "active") return "dropped";
    if (this.#persistentNodes.size + this.#oneShotNodes.size >= MAX_AUDIO_VOICES) {
      return "failed";
    }
    const node = context.createBufferSource();
    ...
    if (this.#nextOneShot > 0xffff_ffff_ffff_ffffn) {
      throw new Error("Voplay one-shot audio identity exhausted");
    }
    const id = this.#nextOneShot;
    this.#nextOneShot += 1n;
    node.onended = () => {
      if (this.#oneShotNodes.delete(id)) this.#oneShotsFinished += 1n;
      this.#syncBusMixState();
      node.disconnect();
      for (const owned of nodes) owned.disconnect();
    };
  ...
  #connectSpatial(
    source: AudioNode,
    bus: AudioNode,
    spatial: BrowserSpatialSource | null,
  ): AudioNode[] {
    const context = this.#audioContext;
    ...
    const position = spatial.position.map((component) => component / 1000) as [
      number, number, number,
    ];
    const compatiblePanner = panner as PannerNode & {
    ...
    if (
      compatiblePanner.positionX !== undefined
  ...
  #decodeAsset(asset: GenerationalHandle): Promise<AudioBuffer> {
    ...
    const context = this.#audioContext;
    if (context === null) return Promise.reject(new Error("Voplay audio device is unavailable"));
    ...
    const raw = resident === undefined
      ...
      : context.decodeAudioData(resident.bytes.slice().buffer as ArrayBuffer);
    const record: DecodedAudioAsset = {
      promise: raw,
      bytes: 0,
    ...
    void pending.catch(() => {
      if (this.#decodedAssets.get(key) === record) this.#dropDecodedAsset(key);
    });
    return pending;
  }

  #takeDecodedUse(): bigint {
    const use = this.#decodedUseClock;
    if (use >= 0xffff_ffff_ffff_ffffn) {
      const ordered = [...this.#decodedAssets.entries()].sort(
        (left, right) => left[1].lastUse < right[1].lastUse ? -1 : 1,
      );
      let next = 1n;
      for (const [, record] of ordered) record.lastUse = next++;
  ...
  #evictDecodedAssets(protectedKey: string): void {
    while (this.#decodedAudioBytes > MAX_DECODED_AUDIO_BYTES) {
      let candidate: [string, DecodedAudioAsset] | null = null;
      for (const entry of this.#decodedAssets) {
        if (entry[0] === protectedKey || entry[1].bytes === 0) continue;
  ...
  #dropDecodedAsset(key: string): void {
  ...
  #clearDecodedAssets(): void {
    this.#decodedAssets.clear();
    this.#decodedAudioBytes = 0;
    this.#decodedUseClock = 1n;
  }

  async #applyAudioAsset(packet: FrameworkPacket): Promise<void> {
    const { header, payload } = packet;
    if (
      payload.byteLength < 25
      || payload[0] !== 0x56
      || payload[1] !== 0x50
      || payload[2] !== 0x41
      || payload[3] !== 0x32
    ...
    if (action === 1 && byteLength > 0) {
      const nextBytes = this.#audioAssetBytes
        ...
        + byteLength;
      if (
        (current === undefined && this.#audioAssets.size >= MAX_AUDIO_ASSETS)
      ...
      if (this.#deviceState === "active") {
        this.#capturePersistentRecoveryOffsets();
        for (const source of physicalSources) {
          if (handleKey(source.asset) !== key) continue;
          const sourceKey = handleKey(source.handle);
          try {
            await this.#startPersistent(
              source,
              false,
              this.#recoveryOffsetsMillis.has(sourceKey),
            );
    ...
    await this.#reply(packet, MessageKind.AudioAssetAck, new Uint8Array(), {
      commitId: identity,
      baseRevision: 0n,
  ...
  #stopOneShotsForAsset(asset: GenerationalHandle): void {
    for (const [id, record] of this.#oneShotNodes) {
      if (!sameHandle(record.asset, asset)) continue;
      record.source.onended = null;
      try { record.source.stop(); } catch {}
      record.source.disconnect();
      for (const node of record.nodes) node.disconnect();
  ...
  #syncInteractiveOutput(): void {
    const interactive = this.#isInteractive();
    if (interactive !== this.#interactive) this.#interactive = interactive;
    this.#syncBusMixState();
  }

  #syncBusMixState(): void {
    const buses = [
      ...this.#control.buses,
      ...this.#control.retiringBuses.map((retiring) => retiring.value),
    ];
    const byKey = new Map(buses.map((bus) => [handleKey(bus.handle), bus]));
    const isDescendantOrSame = (
      candidate: GenerationalHandle,
      ancestor: GenerationalHandle,
    ): boolean => {
      let cursor = byKey.get(handleKey(candidate));
      while (cursor !== undefined) {
        if (sameHandle(cursor.handle, ancestor)) return true;
    ...
    const audible = (bus: BrowserAudioBus): boolean =>
      !soloActive || soloBuses.some((solo) => isDescendantOrSame(bus.handle, solo.handle));
    const triggerActive = (trigger: GenerationalHandle): boolean =>
      voiceBuses.some((bus) => {
    ...
    for (const voice of voices) {
      ...
      for (const rule of ducking) {
        if (
          isDescendantOrSame(voice.bus, rule.target)
  ...
  #applyListener(listener: BrowserAudioListener): void {
    const context = this.#audioContext;
    if (context === null) return;
    const position = listener.position.map((component) => component / 1000) as [
      number, number, number,
    ];
    const rightLength = Math.hypot(...listener.right);
    const right = listener.right.map((component) => component / rightLength) as [
      number, number, number,
    ];
    const referenceUp: [number, number, number] =
      Math.abs(right[1]) < 0.99 ? [0, 1, 0] : [0, 0, 1];
    ...
    const forward = rawForward.map((component) => component / forwardLength) as [
      number, number, number,
    ];
    ...
    if (
      listenerNode.positionX !== undefined
    ...
    if (
      listenerNode.forwardX !== undefined
  ...
  #releaseReadyRetirements(): boolean {
    const domainFrame = this.#audioDomainFrame();
    const ready = (fence: BrowserRetirementFence): boolean =>
      fence.renderRevision === 0n
      && fence.eventSequence === 0n
      && domainFrame >= fence.domainFrame
      && this.#observedAudioEventSequence >= fence.audioEventSequence;
    const retiringBuses = this.#control.retiringBuses.filter(
      (retiring) => !ready(retiring.fence),
    );
    const retiringSources = this.#control.retiringSources.filter(
      (retiring) => !ready(retiring.fence),
    );
    if (
      retiringBuses.length === this.#control.retiringBuses.length
      && retiringSources.length === this.#control.retiringSources.length
    ) {
      return false;
    }
    const retainedSources = new Set(
      retiringSources.map((retiring) => handleKey(retiring.value.handle)),
    );
  ...
  #validateEnvelope(packet: FrameworkPacket): void {
    ...
    if (lane === null) throw new Error("Voplay audio lane is closed");
  ...
  async #publishRealizationResults(source: FrameworkPacket): Promise<void> {
    const controls = [
    ...
    await this.#reply(source, MessageKind.ControlRealizationResult, payload, {
      requiredControlRevision: source.header.newRevision,
    });
  }

  async #publishControlObservation(): Promise<void> {
    const source = this.#lastControl;
    if (source === null) return;
    const domainFrame = this.#audioDomainFrame();
    const key =
      `${source.header.commitId}:${source.header.newRevision}:${domainFrame}:${this.#observedAudioEventSequence}`;
    if (key === this.#lastObservedProgress) return;
    ...
    await this.#reply(source, MessageKind.ControlObserved, payload, {
      requiredControlRevision: source.header.newRevision,
    });
    ...
    while (this.#publishedObservations.size > MAX_AUDIO_OBSERVATION_HISTORY) {
      const oldest = this.#publishedObservations.values().next().value as string | undefined;
  ...
  #acceptControlObservedAck(packet: FrameworkPacket): void {
    const source = this.#lastControl;
  ...
  #audioDomainFrame(): bigint {
    const context = this.#audioContext;
    const frame = this.#audioFrameOffset + (context === null ? 0n : audioContextFrame(context));
    if (frame > MAX_U64) throw new Error("Voplay audio domain frame exhausted");
    return frame;
  }

  #captureAudioFrameOffset(): void {
    const context = this.#audioContext;
    if (context === null) return;
  ...
  #capturePersistentRecoveryOffsets(): void {
    const context = this.#audioContext;
    if (context === null) return;
    for (const [key, record] of this.#persistentNodes) {
      ...
      this.#recoveryOffsetsMillis.set(
        key,
        BigInt(Math.max(0, Math.floor(position * 1000))),
      );
    }
  }

  #requireRunning(engine: GenerationalHandle): void {
  ...
  async #reply(
    source: FrameworkPacket,
    kind: MessageKind,
    payload: Uint8Array,
    overrides: Partial<Omit<FrameworkPacket["header"], "kind" | "payloadLen" | "engine" | "channelEpoch" | "sequence">> = {},
  ): Promise<void> {
    const lane = this.#lane;
    if (lane === null) throw new Error("Voplay audio lane is closed");
...
function decodeAudioControl(packet: FrameworkPacket): BrowserAudioControl {
  const { header, payload } = packet;
  if (
    header.newRevision === 0n
    || header.commitId === 0n
  ...
  const buses: BrowserAudioBus[] = [];
  const sources: BrowserPersistentSource[] = [];
  const retiringBuses: BrowserRetiringAudioBus[] = [];
  const retiringSources: BrowserRetiringAudioSource[] = [];
  const entries: BrowserAudioControlEntry[] = [];
  for (let index = 0; index < count; index += 1) {
    ...
    if (identities.has(identity)) throw new Error("duplicate Voplay audio control entry");
    identities.add(identity);
    const descriptorLength = view.getUint32(offset + 12, true);
    const fence: BrowserRetirementFence = {
    ...
    if (kind === 3) {
      ...
      if (state === 1) buses.push(value);
      else retiringBuses.push({ value, fence });
    } else {
      const value = decodeSourceDescriptor(descriptor, header.engine, handle);
      if (state === 1) sources.push(value);
      else retiringSources.push({ value, fence });
    }
    offset += descriptorLength;
  }
  if (offset !== payload.byteLength) throw new Error("trailing Voplay audio control bytes");
  ...
  if (
    duckingRules.length > MAX_AUDIO_DUCKING_RULES
...
function validateBrowserAudioBusTopology(
  buses: readonly BrowserAudioBus[],
  requireSingleRoot: boolean,
): void {
  const byKey = new Map(buses.map((bus) => [handleKey(bus.handle), bus]));
  if (
    byKey.size !== buses.length
  ...
  for (const bus of buses) {
    ...
    let cursor: BrowserAudioBus | undefined = bus;
...
function isBrowserAudioTombstoneUpdate(
  current: BrowserAudioControl,
  incoming: BrowserAudioControl,
): boolean {
  ...
  for (const entry of current.entries) {
    ...
    if (!sameBytes(entry.descriptor, incomingEntry.descriptor)) return false;
    if (entry.state === incomingEntry.state) {
      if (!sameRetirementFence(entry.fence, incomingEntry.fence)) return false;
      continue;
    }
    if (entry.state !== 1 || incomingEntry.state !== 2) return false;
  }
  return incoming.entries.every((entry) =>
    current.entries.some((currentEntry) =>
...
function sameRetirementFence(
  left: BrowserRetirementFence,
  right: BrowserRetirementFence,
): boolean {
...
function decodeBusDescriptor(
  ...
  if (flags > 3) throw new Error("invalid Voplay audio bus flags");
  ...
  const ducking: BrowserAudioDuckingRule[] = [];
  const duckingTargets = new Set<string>();
...
function decodeSourceDescriptor(
  bytes: Uint8Array,
  engine: GenerationalHandle,
  handle: GenerationalHandle,
): BrowserPersistentSource {
  const reader = new AudioReader(bytes);
  ...
  const gain = reader.gain();
  const flags = reader.u8();
  if ((flags & ~3) !== 0) throw new Error("invalid Voplay audio source flags");
  const recovery = reader.u8();
  if (recovery < 1 || recovery > 3) throw new Error("invalid Voplay audio recovery policy");
...
function decodeOneShot(
  ...
  asset: GenerationalHandle;
  bus: GenerationalHandle;
  gain: number;
  spatial: BrowserSpatialSource | null;
} {
  if (
  ...
  const spatial = spatialTag === 0
    ...
    : spatialTag === 1
      ...
      : invalidAudioDescriptor();
  reader.finish();
  if (engine.generation === 0) throw new Error("invalid Voplay audio engine");
...
class AudioReader {
  ...
  prefix(engine: GenerationalHandle, tag: number): void {
    if (
      this.u8() !== 0x56 || this.u8() !== 0x41 || this.u8() !== 0x43
  ...
  stable(kind: number): GenerationalHandle {
  ...
  spatial(): BrowserSpatialSource {
    const position = [this.i32(), this.i32(), this.i32()] as const;
  ...
  listener(): BrowserAudioListener {
    const position = [this.i32(), this.i32(), this.i32()] as const;
    const right = [this.i16(), this.i16(), this.i16()] as const;
  ...
  gain(): number {
    ...
    if (value > 32767) throw new Error("invalid Voplay audio gain");
    return value / 32767;
  }
...
function invalidAudioDescriptor(): never {
...
function validGenerationalHandle(handle: GenerationalHandle): boolean {
  return handle.index !== 0xffff_ffff && handle.generation !== 0;
}

function validateBrowserAudioPermit(bytes: Uint8Array): void {
  ...
  if (
    handles.some((handle) => !validGenerationalHandle(handle))
...
function audioContextFrame(context: AudioContext): bigint {
